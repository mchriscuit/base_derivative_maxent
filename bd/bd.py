...
=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=* '''
class BDMaxEnt():
    def __init__(self, inputs:list, candidates:list,  winner_idxs:list,
               lexemes:list, constraint_names:list, constraint_types:list,
               constraint_definitions:list, weights):
        ...
        self.outputs = [candidates[i][winner_idxs[i]]
        ...
        self.lex2win = {lexemes[i]:winner_idxs[i]
                        for i in range(self.n_forms)}
    ...
    def compute_violation(self, input:str, candidate:str, lexemes:tuple,
                          bases=None, base_lexemes=None):
        ''' Goal:
        ...
        for i,constraint_type in enumerate(self.constraint_types):
            ...
            if constraint_type == 'M':
                ...
                violation[i] = sum(1 for _ in re.finditer(pattern,
                                                        parsed_candidate))
